...
export class ApiService {
  ...
  async get<ApiResponse>(url: string, options?: {
    ...
    try {
      ...
      console.log(`SENDING GET TO : ${environment.API_URL}${url}`)
  ...
  async addAuthHeader(options: {
    ...
    const tokenType: string = this.userService.TokenType;
    const token: string = this.userService.Token;
    if (tokenType && token) {
      if (!options) {
        options = {headers: new HttpHeaders().append('Authorization', `${this.userService.TokenType} ${this.userService.Token}`)};
      } else {
        options.headers = new HttpHeaders().append('Authorization', `${this.userService.TokenType} ${this.userService.Token}`);
      }
    }
